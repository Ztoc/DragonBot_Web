...
import { ImageSliceType, LeagueSliceType, SkinSliceType, TurboSliceType, UserSliceType } from "../types/store.ts";
...
const RootLayout = () => {
    ...
    if (!user.dataRequested) {
      ...
      user.websocket.on("receive-user", (udata: UserWebhookData) => {
        if (udata.success) {
          ...
          if (udata.turbo != undefined && udata.turbo.length > 0) dispatch(setAvailableTurbos(udata.turbo));
      ...
      user.websocket.on("topSquads", (data: { success: boolean; squads: any }) => {
        if (data.success) dispatch(setTopSquad(data.squads));
      });
      user.websocket.on("squadData", (data: { success: boolean; squad: squadDataLeague }) => {
        if (data.success) dispatch(selectSquad(data.squad));
      });
      ...
      user.websocket.on(
        "squadSuccess",
        (data: { success: boolean; message: string; type: "joined" | "left"; squad: squadDataLeague }) => {
          if (data.success) {
            if (data.type == "joined") {
              navigate(-1);
  ...
  useEffect(() => {}, [turbo.turboMode]);
  useEffect(() => {
    if (user.data != null && user.data?.status == "suspended") navigate("/banned");
  }, [user.data?.status]);
  useEffect(() => {
    loadDragonImages();
  }, []);
