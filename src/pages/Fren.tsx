  ...
  return TOY_IMAGE && fren.haveData ? (
    <div className="fren-zone-container  relative">
      ...
      <div className="relative z-[10]">
        ...
        <div
          className="fren-info animate__animated animate__fadeIn blur-round-border-bg"
          onClick={() => navigate("/top-fren")}
        >
          ...
          {OPEN_IMG ? (
            <img className="fren-opener-arrow opacity-less mr-1" src={OPEN_IMG?.img.src} alt="opener" />
          ) : null}
        </div>
        <p className="fren-bonues-title animate__animated animate__fadeIn animate__slow">Invite frens to get bonuses</p>
        <div
          className="fren-bonues animate__animated animate__fadeIn animate__slow blur-round-border-bg"
          style={
            {
              "--angle": "160deg",
        ...
        <div
          className="blur-round-border-bg"
          style={
            {
              "--angle": "125deg",
            } as React.CSSProperties
              ...
              fren.list.map((fren: frenData) => {
                return (
                  <Friend
                    key={fren.iuser.id}
                    id={fren.iuser.tg_id}
                    fName={fren.iuser?.fName}
                    lName={fren.iuser?.lName}
                    league={fren.iuser?.league?.preset}
                    username={fren.iuser?.username ?? null}
                    balance={fren.iuser?.balance ?? "0"}
                    earned={fren.earned}
                    is_premium={fren.is_premium}
                  />
                );
              })
            ) : (
