...
const imageSlice = createSlice({
  ...
  reducers: {
    ...
    addDailyBoosterImages: (
      state,
      action: { payload: { name: DailyBoosterImageTypes; img: any; type: "big" | "small" } }
    ) => {
    ...
    addDragonImages: (state, action) => {
      state.dragon.push(action.payload);
    },
    ...
    setDragonDone: (state, action) => {
      state.isDragonDone = action.payload;
    },
  },
});
